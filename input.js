...
module.exports = function(RED) {
  ...
  function bypassQueue(smq, context, node, falando) {
  ...
  function simpleMessageQueueNode(node, msg) {

        var smq = {};
        smq.firstMessageBypass = node.firstMessageBypass || false;
        smq.bypassInterval = node.bypassInterval || 10000;
        smq.isBusy = false;
        smq.bypassTimer = null;
        //var node = this;
        ...
        if (msg.hasOwnProperty("reset")) {
            ...
            node.send(portOut(msg,userFlow.falando));
          ...
          context.queue = context.queue.filter(function(x) {
            ...
            if(context.queue.length > 0) {
                var m = context.queue.shift();
                m["_queueCount"] = context.queue.length;
                  node.send(portOut(m,userFlow.falando));
            stopBypassTimer(smq);
            bypassQueue(smq, context, node, userFlow.falando);
            } else {
              setBusyFalse(smq);
            }
        } else {
  ...
  function InputNode(n) {
      ...
      if (RED.settings.httpNodeRoot !== false) {
          ...
          this.propertyTypeFilter = n.propertyTypeFilter;

          this.firstMessageBypass = n.firstMessageBypass || false;
		      this.bypassInterval = n.bypassInterval || 0;
  ...
  RED.nodes.registerType("In msg",InputNode);
  ...
  RED.nodes.registerType("Out msg",OutputNode);
}